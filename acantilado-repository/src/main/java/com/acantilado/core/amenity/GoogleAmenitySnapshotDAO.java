...
public class GoogleAmenitySnapshotDAO extends AbstractDAO<GoogleAmenitySnapshot> {
  ...
  /**
   * Find the most recent snapshot for a given place ID This is the most commonly used query for
   * checking if data has changed
   */
  public Optional<GoogleAmenitySnapshot> findLatestByPlaceId(String placeId) {
    List<GoogleAmenitySnapshot> results =
        currentSession()
            .createQuery(
  ...
  /**
   * Find all snapshots for a given place ID, ordered by lastSeen DESC Used for migration and
   * historical analysis
   */
  ...
  /**
   * Find snapshots within a time range for a given place ID Useful for analyzing changes over a
   * specific period
   */
  public List<GoogleAmenitySnapshot> findByPlaceIdAndTimeRange(
      String placeId, Instant startTime, Instant endTime) {
    return currentSession()
        .createQuery(
            "SELECT s FROM GoogleAmenitySnapshot s "
                + "WHERE s.placeId = :placeId "
  ...
  /**
   * Find all snapshots for a given chain within a time range Useful for analyzing chain-wide trends
   */
  public List<GoogleAmenitySnapshot> findByChainAndTimeRange(
      AcantiladoAmenityChain chain, Instant startTime, Instant endTime) {
    return currentSession()
        .createQuery(
            "SELECT s FROM GoogleAmenitySnapshot s "
  ...
  /**
   * Find snapshots where status changed to a specific status Useful for detecting closures or
   * reopenings
   */
  public List<GoogleAmenitySnapshot> findByStatusChange(GoogleAmenityStatus status, Instant since) {
    return currentSession()
        .createQuery(
            "SELECT s FROM GoogleAmenitySnapshot s "
  ...
  /**
   * Find amenities that haven't been seen since a certain time Useful for detecting potentially
   * closed or missing amenities
   */
  ...
  public long countByPlaceId(String placeId) {
    return currentSession()
        .createQuery(
            "SELECT COUNT(s) FROM GoogleAmenitySnapshot s WHERE s.placeId = :placeId", Long.class)
        .setParameter("placeId", placeId)
        .getSingleResult();
  }

  /** Delete all snapshots for a given place ID Used during place ID migration */
  ...
  /**
   * Find snapshots with rating changes greater than a threshold Useful for detecting significant
   * quality changes
   */
  public List<GoogleAmenitySnapshot> findSignificantRatingChanges(double minChange, Instant since) {
