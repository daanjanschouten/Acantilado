...
public class GoogleAmenityDAO extends AbstractDAO<GoogleAmenity> {
  ...
  public GoogleAmenity merge(GoogleAmenity amenity) {
    return currentSession().merge(amenity);
  }
  ...
  public List<GoogleAmenity> findByType(GoogleAmenityCategory type) {
    return currentSession()
        .createQuery(
            "SELECT a FROM GoogleAmenity a WHERE a.chain.amenityType = :type", GoogleAmenity.class)
        .setParameter("type", type)
        .getResultList();
  }

  /**
   * Find amenities within a bounding box and optionally filter by chain Used for detecting
   * potential Place ID changes
   */
  public List<GoogleAmenity> findByLocationAndChain(
      double latitude, double longitude, double delta, AcantiladoAmenityChain chain) {
    return currentSession()
        .createQuery(
  ...
  public List<GoogleAmenity> findByLocation(double latitude, double longitude, double delta) {
    return currentSession()
        .createQuery(
            "SELECT a FROM GoogleAmenity a "
                + "WHERE a.latitude BETWEEN :minLat AND :maxLat "
  ...
  public Optional<GoogleAmenity> findByPreviousPlaceId(String previousPlaceId) {
    List<GoogleAmenity> results =
        currentSession()
            .createQuery(
                "SELECT a FROM GoogleAmenity a WHERE a.previousPlaceId = :previousPlaceId",
                GoogleAmenity.class)
