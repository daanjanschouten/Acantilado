...
public class GoogleAmenityResource {
  ...
  public Response getByType(@PathParam("type") String typeName) {
    try {
      GoogleAmenityCategory type = GoogleAmenityCategory.valueOf(typeName.toUpperCase());
  ...
  /**
   * GET /amenities/location?lat={lat}&lng={lng}&delta={delta}&chain={chain} Find amenities near a
   * location Example: /amenities/location?lat=40.4168&lng=-3.7038&delta=0.01&chain=CARREFOUR
   */
  @GET
  @Path("/location")
  @UnitOfWork
  ...
  /**
   * GET /amenities/previous/{previousPlaceId} Find amenity by its previous Place ID (for tracking
   * migrations)
   */
  @GET
  ...
  /** GET /amenities/stats Get statistics about amenities */
  @GET
  @Path("/stats")
  @UnitOfWork
