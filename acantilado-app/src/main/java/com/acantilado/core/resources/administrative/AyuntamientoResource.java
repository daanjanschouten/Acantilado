...
public class AyuntamientoResource {
  ...
  @GET
  @Path("/getByProvinceId/{provinceId}")
  @UnitOfWork
  public List<Ayuntamiento> getByProvinceId(@PathParam("provinceId") String provinceId) {
    return ayuntamientoDao.findByProvinceId(provinceId);
  }

  private Ayuntamiento findSafely(String ayuntamientoId) {
    return ayuntamientoDao
        .findById(ayuntamientoId)
        .orElseThrow(() -> new NotFoundException("No such ayuntamiento." + ayuntamientoId));
  }
