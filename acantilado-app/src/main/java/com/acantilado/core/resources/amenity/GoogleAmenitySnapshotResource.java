...
public class GoogleAmenitySnapshotResource {
  ...
  /** GET /snapshots/latest/{placeId} Get the most recent snapshot for a place */
  @GET
  @Path("/latest/{placeId}")
  @UnitOfWork
  ...
  /** GET /snapshots/{placeId} Get all snapshots for a place (ordered by most recent) */
  @GET
  @Path("/{placeId}")
  @UnitOfWork
  ...
  /**
   * GET /snapshots/{placeId}/range?start={iso}&end={iso} Get snapshots within a time range Example:
   * /snapshots/ChIJ123/range?start=2025-01-01T00:00:00Z&end=2025-12-31T23:59:59Z
   */
  @GET
  @Path("/{placeId}/range")
  @UnitOfWork
  public Response getByTimeRange(
    ...
    try {
      ...
      List<GoogleAmenitySnapshot> snapshots =
          snapshotDAO.findByPlaceIdAndTimeRange(placeId, start, end);
      return Response.ok(snapshots).build();
    } catch (Exception e) {
      return Response.status(Response.Status.BAD_REQUEST)
  ...
  /**
   * GET /snapshots/chain/{chain}/range?start={iso}&end={iso} Get snapshots for a chain within a
   * time range Example:
   * /snapshots/chain/CARREFOUR/range?start=2025-01-01T00:00:00Z&end=2025-12-31T23:59:59Z
   */
  @GET
  ...
  public Response getByChainAndTimeRange(
    ...
    try {
      ...
      List<GoogleAmenitySnapshot> snapshots =
          snapshotDAO.findByChainAndTimeRange(chain, start, end);
      return Response.ok(snapshots).build();
  ...
  /**
   * GET /snapshots/status/{status}?days={days} Find snapshots where status changed to a specific
   * status Example: /snapshots/status/CLOSED_PERMANENTLY?days=30
   */
  @GET
  @Path("/status/{status}")
  @UnitOfWork
  public Response getByStatusChange(
      @PathParam("status") String statusName, @QueryParam("days") @DefaultValue("30") int daysAgo) {
  ...
  /**
   * GET /snapshots/stale?days={days} Find amenities that haven't been seen in X days Example:
   * /snapshots/stale?days=7
   */
  @GET
  @Path("/stale")
  @UnitOfWork
  ...
  /**
   * GET /snapshots/rating-changes?minChange={change}&days={days} Find significant rating changes
  ...
  public Response getRatingChanges(
    ...
    List<GoogleAmenitySnapshot> snapshots =
        snapshotDAO.findSignificantRatingChanges(minChange, since);
    return Response.ok(snapshots).build();
  }

  /** GET /snapshots/stats Get statistics about snapshots */
  @GET
  @Path("/stats")
  @UnitOfWork
  public Response getStats() {
    ...
    return Response.ok(Map.of("totalSnapshots", totalCount)).build();
  }

  /** GET /snapshots/{placeId}/count Count snapshots for a specific place */
  @GET
  @Path("/{placeId}/count")
  @UnitOfWork
