...
@NamedQueries({
  @NamedQuery(
      name =
          "com.schouten.core.properties.idealista.IdealistaPropertyPriceRecord.findByPropertyCode",
      query =
          "SELECT p FROM IdealistaPropertyPriceRecord p WHERE p.propertyCode = :propertyCode ORDER BY p.recordedAt DESC"),
  @NamedQuery(
      name =
...
public class IdealistaPropertyPriceRecord extends IdealistaPriceRecordBase {
  ...
  @JoinColumn(
      name = "property_code",
      referencedColumnName = "property_code",
      insertable = false,
      updatable = false)
  ...
  public IdealistaProperty getProperty() {
    return property;
  }

  public void setProperty(IdealistaProperty property) {
