...
public class LocationMappingMerchant {
  ...
  public LocationMappingMerchant(
      IdealistaLocationMappingDAO mappingDAO, SessionFactory sessionFactory) {
  ...
  public void exportMappings(Provincia provincia) {
    Set<IdealistaLocationMapping> provinceMappings =
    ...
    try {
      ...
      LOGGER.info(
          "Exported {} mappings for province {} to {}",
          provinceMappings.size(),
          provincia.getName(),
  ...
  public boolean importMappings(Provincia provincia) {
    ...
    try {
      List<IdealistaLocationMapping> mappings =
          mapper.readValue(inputFile, new TypeReference<List<IdealistaLocationMapping>>() {});
      ...
      for (IdealistaLocationMapping mapping : mappings) {
        executeRunnableInSessionWithTransaction(
            sessionFactory,
            () -> {
              List<IdealistaLocationMapping> existing =
                  mappingDAO.findByIdealistaLocationId(mapping.getIdealistaLocationId());
      ...
      LOGGER.info(
          "Imported {} mappings for province {} from {}",
          mappings.size(),
          provincia.getName(),
