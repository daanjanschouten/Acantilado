 ...
 *
 * <p>NATCODE Format: CC AA PP PPMMMM (11 digits) - CC (positions 0-1): Country code = "34" (Spain)
 * - AA (positions 2-3): Autonomous Community code - PP (positions 4-5): Province code - PPMMMM
 * (positions 6-10): Full 5-digit INE municipality code
 *
 * <p>Examples: - Municipality: 34160101017 -> Country="34", CCAA="16", Province="01", INE="01017" -
 * Province: 34160100000 -> Country="34", CCAA="16", Province="01", INE="00000" - CCAA: 34160000000
 * -> Country="34", CCAA="16", Province="00", INE="00000"
 *
 * <p>All IDs are returned as Strings to preserve leading zeros.
 */
public final class NatCode {
  ...
  private static final int NATCODE_LENGTH =
      COUNTRY_CODE_LENGTH + CCAA_CODE_LENGTH + PROVINCE_CODE_LENGTH + INE_CODE_LENGTH;
  ...
  public static NatCode parse(String natCode) {
    ...
    if (natCode.length() != NATCODE_LENGTH) {
      throw new IllegalArgumentException(
          String.format(
              "Invalid NATCODE length: expected %d, got %d for NATCODE '%s'",
    ...
    if (!EXPECTED_COUNTRY_CODE.equals(countryCode)) {
      throw new IllegalArgumentException(
          String.format(
              "Invalid country code in NATCODE '%s': expected %s, got %s",
              natCode, EXPECTED_COUNTRY_CODE, countryCode));
    }

    LOGGER.debug(
        "Parsed NATCODE {}: Country={}, CCAA={}, Provincia={}, INE={}",
        natCode,
        countryCode,
        ccaaCode,
        provinciaCode,
