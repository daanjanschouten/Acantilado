    ...
    try {
      ...
      if (geometry == null) {
        LOGGER.warn(
            ...
            feature.toString().substring(0, Math.min(200, feature.toString().length())));
      ...
      if (name == null || name.trim().isEmpty()) {
        LOGGER.warn(
            "Barrio missing name, skipping. Properties: {}",
            properties.toString().substring(0, Math.min(200, properties.toString().length())));
        return Optional.empty();
      }
