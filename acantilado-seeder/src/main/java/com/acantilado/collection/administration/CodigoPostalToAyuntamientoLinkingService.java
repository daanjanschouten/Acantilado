...
public final class CodigoPostalToAyuntamientoLinkingService extends CollectorService {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(CodigoPostalToAyuntamientoLinkingService.class);
  ...
  public CodigoPostalToAyuntamientoLinkingService(
  ...
  public void seed() {
    ...
    collectPostCodesByProvinceIds(sessionFactory, codigoPostalDAO)
        .forEach(
            (provinciaId, postalCodes) -> linkPostCodesToAyuntamientos(postalCodes, provinciaId));
    LOGGER.info("Finished linking postal codes to ayuntamientos");
  }

  /**
   * Linking is based only on proximity, not intersection. This is because these links are purely
   * used to filter down possible postcodes for a given real estate listing, as Idealista provides a
   * reliable ayuntamiento.
   */
  private void linkPostCodesToAyuntamientos(Set<String> postalCodeIds, String provinciaId) {
    ...
    try {
      ...
      LOGGER.info(
          "Linking {} postcodes to {} ayuntamientos for provincia {}",
          postalCodeIds.size(),
          ayuntamientos.size(),
          provinciaId);
      ...
      for (String postalCodeId : postalCodeIds) {
        ...
        for (CodigoPostal cp : codigosPostales) {
          for (Ayuntamiento ayuntamiento : ayuntamientos) {
            try {
              ...
              LOGGER.error(
                  "Error linking postal code {} to municipality {} ({})",
                  postalCodeId,
        ...
        if (linksForThisPostalCode == 0) {
          LOGGER.error(
              "No ayuntamientos found for postal code {} at proximity setting {}",
              postalCodeId,
              PROXIMITY_DEGREES_THRESHOLD);
      ...
      double averageLinkCount =
          !postalCodeIds.isEmpty() ? (double) totalLinksCreated / postalCodeIds.size() : 0;
      LOGGER.debug(
          "Created {} links with {} links on average", totalLinksCreated, averageLinkCount);
    } catch (Exception e) {
      if (transaction != null) {
  ...
  private void logClosestAyuntamiento(CodigoPostal codigoPostal, List<Ayuntamiento> ayuntamientos) {
    ...
    throw new RuntimeException(
        "Postal code "
            ...
            + "municipality. Closest is "
            + closestAyuntamiento
            + " at distance of "
            + (int) (minDistance * 111000)
            + ".");
  }

  private static List<CodigoPostal> getCodigosPostales(
      String postalCodeId, CodigoPostalDAO codigoPostalDAO) {
  ...
  private static Map<String, Set<String>> collectPostCodesByProvinceIds(
      SessionFactory sessionFactory, CodigoPostalDAO codigoPostalDAO) {
    Session session = sessionFactory.openSession();
    ManagedSessionContext.bind(session);

    try {
      return codigoPostalDAO.findAll().stream()
