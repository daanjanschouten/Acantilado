...
class AcantiladoLocationTest {
    ...
    private final Ayuntamiento ayuntamiento = new Ayuntamiento(
            ...
            geometry);
    private final CodigoPostal codigoPostal = new CodigoPostal("someCodigoPostal", geometry);
    ...
    void beforeEach() {
        ...
        when(codigoPostalDAO.findById("someCodigoPostal")).thenReturn(Optional.of(codigoPostal));
        when(codigoPostalDAO.findById("noneCodigoPostal")).thenReturn(Optional.empty());
    ...
    void buildsLocationIdentifier() {
        ...
        Assertions.assertEquals("100-someCodigoPostal-1111", location.getIdentifier());
    }

    @Test
    void buildsLocationIdentifierWithoutBarrio() {
        location = new AcantiladoLocation(ayuntamiento, codigoPostal);
        Assertions.assertEquals("100-someCodigoPostal-XXX", location.getIdentifier());
    }

    @Test
    void extractsFromLocationIdentifier() {
        location = AcantiladoLocation.fromLocationIdentifier("100-someCodigoPostal-1111", ayuntamientoDAO, codigoPostalDAO, barrioDAO);
    ...
    void extractsFromLocationIdentifierThrowsInvalidStateException() {
        Assertions.assertThrows(
                IllegalStateException.class, () -> AcantiladoLocation.fromLocationIdentifier(
                        "101-someCodigoPostal-1111", ayuntamientoDAO, codigoPostalDAO, barrioDAO)
        );
    }
}