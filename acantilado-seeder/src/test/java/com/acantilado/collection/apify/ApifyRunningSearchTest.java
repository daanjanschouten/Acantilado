...
class ApifyRunningSearchTest {
  ...
  void testThreeArgConstructor_parameterOrderCorrect() {
    ...
    ApifyRunningSearch<String> search =
        new ApifyRunningSearch<>(testRequest, expectedRunId, expectedDatasetId);

    // Then - Verify fields are assigned correctly (not swapped)
    assertEquals(
        expectedRunId, search.getRunId(), "RunId should match the value passed as 2nd parameter");
    assertEquals(
        expectedDatasetId,
        search.getDatasetId(),
  ...
  void testFourArgConstructor_allFieldsAssigned() {
    ...
    // When
    ApifyRunningSearch<String> search =
        new ApifyRunningSearch<>(testRequest, expectedStatus, expectedRunId, expectedDatasetId);
  ...
  void testWithStatus_preservesAllFields() {
    ...
    ApifyRunningSearch<String> original = new ApifyRunningSearch<>(request, runId, datasetId);
  ...
  void testWithStatus_doesNotMutateOriginal() {
    // Given
    ApifyRunningSearch<String> original =
        new ApifyRunningSearch<>("request", "run-id", "dataset-id");
  ...
  void testRealWorldScenario_apifyResponseMapping() {
    ...
    assertEquals(
        actualDatasetIdFromApi,
        datasetIdForResultsFetch,
